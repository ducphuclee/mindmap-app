...
export default function ColorPicker({ value, onChange, label }: ColorPickerProps) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      />
      {open && (
        <div className="absolute bottom-full left-1/2 z-50 mb-2 -translate-x-1/2 rounded-[12px] border border-gray-200 bg-white p-2 shadow-md">
            ...
            {PRESET_COLORS.map((color) => (
              <button
                ...
                className={`h-5 w-5 rounded border ${
                  value === color ? 'ring-2 ring-[#3b9eff]' : 'border-gray-200'
                }`}
