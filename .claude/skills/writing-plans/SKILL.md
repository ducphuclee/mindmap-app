...
- **Task Planning:** Save/attach implementation plan to Venus task when task-bound for persistent reference and team visibility using `write_task_section` (section='plan').
- **Document Storage:** Plans are written to `materials/plans/` — ingest pipeline indexes them into Venus automatically. Only call `create_document` if the plan needs immediate Venus availability before next ingest run.
